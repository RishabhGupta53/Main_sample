...
resource "azurerm_application_gateway" "agw" {
  ...
  dynamic "backend_address_pool" {
    ...
    content {
      name         = each.key
      ip_addresses = [for vm in backend_address_pool.value : data.azurerm_network_interface.vm_nics["${each.key}_${vm}"].private_ip_address]
    }
  }
  ...
  dynamic "backend_address_pool" {
    ...
    content {
      name  = each.key
      fqdns = [for app in backend_address_pool.value : data.azurerm_app_service.app_services["${each.key}_${app}"].default_site_hostname]
    }
  }
